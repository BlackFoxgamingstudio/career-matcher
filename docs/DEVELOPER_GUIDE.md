# Developer Guide: Sovereign Career Matcher
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/career-matcher.git
cd career-matcher
...
```bash
export SBB_NLP_MODEL=your_value_here
export SBB_JOB_BOARD_API_KEY=your_value_here
export SBB_ATS_RULES_PATH=your_value_here
export SBB_SIMILARITY_THRESHOLD=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-career-matcher)
...
```bash
# Build
docker build -t sovereign-career-matcher:dev .

# Run with env file
docker run --env-file .env -p 8796:8796 sovereign-career-matcher:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-career-matcher
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8796`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-career-matcher",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8796` in use | `lsof -i :8796` | Kill PID or change `SERVICE_PORT` |
